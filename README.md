...
* **🛠️ Interactive Unified Memory CLI Helpers**: Added utility scripts under `scripts/memory/` (`dump_memory.py`, `backup_memory.py`, `restore_memory.py`, `clear_memory.py`) for terminal-based Unified Memory management (creating/updating user profile attributes, dumping database keys) to easily inspect and edit backend state.
