...
def get_github_profile_tool(username: str = None) -> str:
    ...
    try:
        from src.CoreFunctions.Integrations.Github.github_ops import get_github_profile
        res = get_github_profile(username)
        return json.dumps(res, indent=2)
    except Exception as e:
        return f"Error fetching GitHub profile: {e}"

get_github_profile = StructuredTool.from_function(
    func=get_github_profile_tool,
