...
def apply_label_to_emails(
    ...
    """Apply a specific custom label to a batch of emails.
    Provide either a `job_id` (with optional offset/limit) or a specific list of `message_ids`.
    
    Args:
        label_name (str): The name of the label to apply. Created dynamically if it does not exist.
        job_id (str, optional): The job ID returned from fetch_email_ids.
        message_ids (list, optional): List of specific email IDs to label.
    ...
    """
    print(f"\n[DEBUG] 🛠️ Calling Tool: apply_label_to_emails")
    target_ids = []
    ...
    try:
        ...
        return f"Error applying label to emails: {e}"

gmail_worker_tool_apply_label_to_emails = StructuredTool.from_function(
    func=apply_label_to_emails,
    name="apply_label_to_emails",
    description="Apply a specific custom label to a batch of emails."
)
