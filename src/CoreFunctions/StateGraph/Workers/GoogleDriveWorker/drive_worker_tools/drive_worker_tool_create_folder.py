...
def create_drive_folder_tool(name: str, parent_id: str = None, account: str = "personal") -> str:
    ...
    try:
        from src.CoreFunctions.Integrations.GoogleDrive.drive_ops import create_drive_folder
        res = create_drive_folder(name, parent_id, account=account)
        return json.dumps(res, indent=2)
    except Exception as e:
        return f"Error creating folder: {e}"

create_drive_folder_tool = StructuredTool.from_function(
    func=create_drive_folder_tool,
    name="create_drive_folder_tool",
