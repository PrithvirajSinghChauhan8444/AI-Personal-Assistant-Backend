...
def get_drive_about_tool(account: str = "personal") -> str:
    ...
    try:
        from src.CoreFunctions.Integrations.GoogleDrive.drive_ops import get_drive_about
        res = get_drive_about(account=account)
        return json.dumps(res, indent=2)
    except Exception as e:
        return f"Error retrieving Drive storage info: {e}"

get_drive_about_tool = StructuredTool.from_function(
    func=get_drive_about_tool,
    name="get_drive_about_tool",
