...
def upload_drive_file_tool(local_path: str, name: str = None, parent_id: str = None, account: str = "personal") -> str:
    ...
    try:
        from src.CoreFunctions.Integrations.GoogleDrive.drive_ops import upload_drive_file
        res = upload_drive_file(local_path, name, parent_id, account=account)
        return json.dumps(res, indent=2)
    except Exception as e:
        return f"Error uploading file: {e}"

upload_drive_file_tool = StructuredTool.from_function(
    func=upload_drive_file_tool,
    name="upload_drive_file_tool",
