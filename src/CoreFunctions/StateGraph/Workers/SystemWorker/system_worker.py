...
class SystemWorker(BaseWorker):
    name = "SystemWorker"
    description = "Handles OS terminal commands, file management, scripts, system health, task scheduling, alarms, configuration/listing. It is the ONLY worker with local file system write/read permissions."
    ...
    def routing_rules(self) -> List[str]:
        return [
            "**Task Scheduling & Reminders**:\n   - Any requests to schedule tasks, set alarms, list scheduled jobs, cancel schedules, or remind the user of anything in the future MUST be routed to SystemWorker. SystemWorker possesses the tools to handle background job scheduling safely without blocking execution.",
            "**Media Control / Music Playback Workflow**:\n   - Any requests to control playback (play, pause, toggle, next, previous, status check) for Spotify or YouTube Music should be routed to SystemWorker. SystemWorker must run the local helper python scripts using the project's virtual environment interpreter (e.g. `.venv/bin/python3 Skills/MiscWorker/youtube-music-control/scripts/youtube_music_control.py` or `.venv/bin/python3 Skills/SystemWorker/spotify-playback-control/scripts/spotify_playback_control.py`) to execute these actions. Do NOT use system `python3` or try to install packages.",
