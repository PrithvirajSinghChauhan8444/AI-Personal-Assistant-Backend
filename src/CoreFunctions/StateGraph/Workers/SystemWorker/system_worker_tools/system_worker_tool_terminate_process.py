...
def terminate_process_tool(name_or_pid: str) -> str:
    ...
    if verify_password():
        try:
            from src.CoreFunctions.Integrations.System.system_actions import kill_process
            return kill_process(name_or_pid)
        except Exception as e:
            return f"Error: {e}"
    return "❌ Action Cancelled: Incorrect Password."

terminate_process_tool = StructuredTool.from_function(
    func=terminate_process_tool,
    name="terminate_process_tool",
